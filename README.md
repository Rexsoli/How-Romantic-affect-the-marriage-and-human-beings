# How-Romantic and Family Property Distribution affects the Population Evolution 

This program provides a simple rational-agent marriage model. The probability of a couple getting married is based on a wealth-based Fermi-Dirac distribution, where the probability is strongly influenced by the 'temperature,' or the 'cold-blooded' index T. The higher the temperature, the less the wealth factor matters in marriage. Another indicator is the proportion of wealth allocation by parents between boys and girls; the one who receives more wealth holds a certain advantage in the marriage market. Additionally, preferences for the number of children are also included. The marriage preferences of the children born will be influenced by their parents’ preferences, with the wealthier party in the marriage having a greater impact on the children. If the current pairing is unreasonable, then this family's marriage preference will be eliminated (unable to marry and have children), so through natural selection, one can observe the optimal fertility rate and wealth distribution orientation among different social strata in a stable situation.
