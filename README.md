...
This program provides a simple rational-agent marriage model, where couples hope to maximize their household wealth through remarriage. The probability of a couple getting married is based on a wealth-based Fermi-Dirac distribution, where the probability is strongly influenced by the temperature, that is, the "cold-blooded" index T. The higher the temperature, the smaller the proportion of wealth in the remarriage. Another indicator is the ratio of parental wealth allocation between boys and girls; the side that receives more wealth will have an advantage in the marriage market. Additionally, preferences for the number of children are included. The marriage preferences of the children born will be influenced by their parents' preferences, with the wealthier spouse having a greater impact on the children. If the current marriages are unreasonable, the family’s marriage preferences will be eliminated (unable to marry and have children). Therefore, through natural selection, it is possible to observe the optimal fertility rates and wealth distribution orientations in different social strata under stable conditions.
